...
/**
 * Bundles all top-level overlay state (history, settings, delete-confirm, chat)
...
export function useOverlays() {
  ...
  const stack = useMemo(() => [
    ...
    { open: showChat,        close: () => setShowChat(false) },
    { open: !!pendingDelete, close: () => setPendingDelete(null) },
  ], [showHistory, showVoiceMenu, showChat, pendingDelete]);
