    ...
    <main dir={urdu ? "rtl" : "ltr"} className="min-h-dvh bg-[radial-gradient(ellipse_at_top,oklch(0.18_0.05_260),oklch(0.09_0.02_240)_60%)] text-white/90">
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
        <header className="mb-7 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2"><Link to="/business-profile" className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] text-white/70 hover:bg-white/10"><Settings2 className="h-3.5 w-3.5" />{copy.profile}</Link><span className="text-[10px] uppercase tracking-[0.2em] text-cyan-300/70">ALPHA · SAR</span></div>
        </header>

        <div className="mb-7 flex items-start justify-between gap-4"><div className="flex items-start gap-3"><LayoutDashboard className="mt-1 h-6 w-6 text-cyan-300" /><div><h1 className="text-xl font-semibold sm:text-2xl">{copy.title}</h1><p className="mt-1 text-xs leading-relaxed text-white/55 sm:text-sm">{profileName ? `${profileName} · ` : ""}{copy.subtitle}</p></div></div><button type="button" onClick={refresh} aria-label={copy.refresh} className="rounded-lg border border-white/10 bg-white/5 p-2 text-white/60 hover:bg-white/10 hover:text-white"><RefreshCw className="h-4 w-4" /></button></div>
        ...
        <section className="mt-5"><h2 className="mb-3 text-sm font-semibold text-white/80">{copy.quick}</h2><div className="grid gap-2 sm:grid-cols-3"><QuickLink href="/" icon="WA" label={copy.whatsapp} /><QuickLink href="/" icon="@" label={copy.email} /><QuickLink href="/" icon="AI" label={copy.briefing} /></div></section>
      </div>
    </main>
  );
}
...
function QuickLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return <Link to={href} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3 text-xs text-white/75 transition hover:border-cyan-300/25 hover:bg-cyan-300/[0.06]"><span className="grid h-8 w-8 place-items-center rounded-lg bg-cyan-300/10 text-[10px] font-semibold text-cyan-200">{icon}</span><span className="flex-1">{label}</span><ExternalLink className="h-3.5 w-3.5 text-white/30" /></Link>;
}
