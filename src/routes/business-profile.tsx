    ...
    <main className="min-h-dvh bg-[radial-gradient(ellipse_at_top,oklch(0.18_0.05_260),oklch(0.09_0.02_240)_60%)] text-white/90">
      <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6 sm:py-10">
        <header className="mb-6 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2"><Link to="/dashboard" className="inline-flex min-h-11 items-center rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[11px] text-white/70 hover:bg-white/10 touch-manipulation">{urdu ? "ڈیش بورڈ" : "Dashboard"}</Link><span className="text-[10px] uppercase tracking-[0.2em] text-cyan-300/70">SAR · Saudi SME</span></div>
        </header>
        ...
        <section className="glass-card mb-5 rounded-xl p-4 sm:p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className="text-xs text-white/55">{copy.tone}<select value={profile.tone} onChange={(e) => update("tone", e.target.value as BusinessProfile["tone"])} className="mt-1.5 min-h-11 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/40 touch-manipulation"><option value="formal">{copy.formal}</option><option value="friendly">{copy.friendly}</option><option value="direct">{copy.direct}</option></select></label>
        ...
        <section className="glass-card rounded-xl p-4 sm:p-5">
          ...
          <div className="grid gap-2 sm:grid-cols-4">
            ...
            <select value={newReminder.category} onChange={(e) => setNewReminder((v) => ({ ...v, category: e.target.value as BusinessReminder["category"] }))} aria-label={copy.category} className="min-h-11 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-white outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/40 touch-manipulation"><option value="tax">{copy.tax}</option><option value="license">{copy.license}</option><option value="payment">{copy.payment}</option><option value="meeting">{copy.meeting}</option><option value="other">{copy.other}</option></select>
            <select value={newReminder.repeat} onChange={(e) => setNewReminder((v) => ({ ...v, repeat: e.target.value as ReminderRepeat }))} aria-label={copy.repeat} className="min-h-11 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-white outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/40 touch-manipulation"><option value="none">{copy.none}</option><option value="daily">{copy.daily}</option><option value="weekly">{copy.weekly}</option><option value="monthly">{copy.monthly}</option></select>
          </div>
          ...
          <button type="button" onClick={addReminder} disabled={!newReminder.title.trim() || !newReminder.dueDate} className="mt-3 inline-flex min-h-11 items-center gap-1.5 rounded-lg border border-violet-300/35 bg-violet-300/15 px-3 text-xs text-violet-100 disabled:opacity-40 touch-manipulation"><Plus className="h-3.5 w-3.5" />{copy.add}</button>
