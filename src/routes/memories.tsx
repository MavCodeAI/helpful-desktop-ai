...
function MemoriesPage() {
  ...
  const onClear = () => {
    ...
    if (confirm(`Delete all ${items.length} memories? This can't be undone.`)) {
      clearMemories();
      refresh();
    }
  };
      ...
      <div className="mx-auto max-w-2xl px-4 sm:px-6 py-6 sm:py-10">
        <header className="flex items-center justify-between gap-3 mb-6">
          ...
          <div className="text-[10px] uppercase tracking-[0.25em] text-cyan-300/80">
            {items.length}/25 stored
          </div>
        </header>

        <div className="flex items-center gap-2.5 mb-2">
          <Brain className="w-5 h-5 text-violet-300" />
          <h1 className="text-xl sm:text-2xl font-semibold">Memory Manager</h1>
        </div>
        <p className="text-xs sm:text-sm text-white/60 mb-6">
          Facts Alpha remembers about you — saved via <span className="text-white/80">“yaad rakho”</span> or auto-extracted from chats. Fully local to this browser.
        </p>

        {/* Add new */}
        <div className="glass-card rounded-xl p-3 sm:p-4 mb-4">
          <div className="text-[10px] uppercase tracking-widest text-white/60 mb-2">Add a memory</div>
          <div className="flex gap-2">
            <input
              value={draft}
              onChange={(e) => setDraft(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") onAdd(); }}
              placeholder="e.g. I prefer concise answers in Urdu"
        ...
        {items.length === 0 ? (
          <div className="glass-card rounded-xl p-8 text-center">
            ...
            <div className="text-sm text-white/60">No memories yet.</div>
            <div className="text-xs text-white/40 mt-1">
              Say <span className="text-white/70">“yaad rakho …”</span> during a chat, or add one above.
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-xs text-white/50 text-center py-8">No matches.</div>
        ) : (
          <ul className="space-y-2">
            {filtered.map(({ t, i }) => (
              ...
              >
                ...
                <button
                  onClick={() => onRemove(i)}
                  className="opacity-60 hover:opacity-100 hover:text-red-300 transition shrink-0"
